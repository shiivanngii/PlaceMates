 ...
 *
 * Groq API provider — llama-3.3-70b-versatile (or any Groq-hosted model).
 * Uses OpenAI-compatible /v1/chat/completions endpoint.
 *
 * Free tier limits: ~6,000 TPM, 30 RPM.
 */
  ...
  async call(prompt: string, options: LLMCallOptions = {}): Promise<LLMCallResult> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const err = new Error(
          `Groq API ${res.status}: ${body.slice(0, 300)}`
        );
